...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class TestStatus
	{
		...
		internal void SetEntry(ZipEntryM entry)
		{
			entry_ = entry;
			entryValid_ = true;
			bytesTested_ = 0;
	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		public ZipEntryM this[int index] {
			get {
				return (ZipEntryM)entries_[index].Clone();
			}
		}
		...
		public IEnumerator GetEnumerator()
		{
			...
			return new ZipEntryMEnumerator(entries_);
		}
		...
		/// The Zip file has been closed.
		/// </exception>
		public ZipEntryM GetEntry(string name)
		{
			if (isDisposed_) {
				//throw new ObjectDisposedException("ZipFile");
			}
			...
			return (index >= 0) ? (ZipEntryM)entries_[index].Clone() : null;
		}

		/// <summary>
		/// Gets an input stream for reading the given zip entry data in an uncompressed form.
		/// Normally the <see cref="ZipEntryM"/> should be an entry returned by GetEntry().
		/// </summary>
		/// <param name="entry">The <see cref="ZipEntryM"/> to obtain a data <see cref="Stream"/> for</param>
		/// <returns>An input <see cref="Stream"/> containing data for this <see cref="ZipEntryM"/></returns>
		/// <exception cref="ObjectDisposedException">
		...
		/// </exception>
		public Stream GetInputStream(ZipEntryM entry)
		{
			if (entry == null) {
...
//				default:
		...
		long TestLocalHeader(ZipEntryM entry, HeaderTest tests)
		{
			lock (baseStream_) {
				...
				if (testHeader) {
					...
					// Assuming a code page at this point is not valid?  Best is to store the name length in the ZipEntryM probably
...
//							entry.CompressedSize, compressedSize));
		...
		/// <summary>
		/// Get/set the <see cref="IEntryFactory"/> used to generate <see cref="ZipEntryM"/> values
		/// during updates.
		/// </summary>
		public IEntryFactory EntryFactory {
			...
			set {
				if (value == null) {
					updateEntryFactory_ = new ZipEntryMFactory();
				} else {
		...
		public int BufferSize {
			...
			set {
				if (value < 1024) {
					//throw new //nameofnameof//(value), "cannot be below 1024");
				}
		...
		public void BeginUpdate(IArchiveStorage archiveStorage, IDynamicDataSource dataSource)
		{
			...
			foreach (ZipEntryM entry in entries_) {
		...
		public void Add(string fileName, CompressionMethod compressionMethod, bool useUnicodeText)
		{
			...
			if (!ZipEntryM.IsCompressionMethodSupported(compressionMethod)) {
				//throw new //nameofnameof//(compressionMethod));
			}

			CheckUpdating();
			contentsEdited_ = true;

			ZipEntryM entry = EntryFactory.MakeFileEntry(fileName);
		...
		/// </summary>
		/// <param name="fileName">The name of the file to add.</param>
		/// <param name="entryName">The name to use for the <see cref="ZipEntryM"/> on the Zip file created.</param>
		/// <exception cref="ArgumentNullException">Argument supplied is null.</exception>
		...
		public void Add(IStaticDataSource dataSource, string entryName, CompressionMethod compressionMethod)
		{
			...
			ZipEntryM entry = EntryFactory.MakeFileEntry(entryName, false);
			entry.CompressionMethod = compressionMethod;
		...
		public void AddDirectory(string directoryName)
		{
			...
			ZipEntryM dirEntry = EntryFactory.MakeDirectoryEntry(directoryName);
		...
		/* Modify not yet ready for public consumption.
		   ...
		   Safe mode is trivial in this sense.
				public void Modify(ZipEntryM original, ZipEntryM updated)
				{
					if ( original == null ) {
		...
		*/
		...
		/// <summary>
		/// Delete a <see cref="ZipEntryM"/> from the archive.
		/// </summary>
		/// <param name="entry">The entry to delete.</param>
		public void Delete(ZipEntryM entry)
		{
			if (entry == null) {
				//throw new ArgumentNullException(//nameof//(entry));
			}
		...
		{
			ZipEntryM entry = update.OutEntry;
		...
		int WriteCentralDirectoryHeader(ZipEntryM entry)
		{
		...
		int FindExistingUpdate(ZipEntryM entry)
		{
			int result = -1;
		...
		/// <summary>
		/// Get an output stream for the specified <see cref="ZipEntryM"/>
		/// </summary>
		/// <param name="entry">The entry to get an output stream for.</param>
		/// <returns>The output stream obtained for the entry.</returns>
		Stream GetOutputStream(ZipEntryM entry)
		{
...
//				default:
		...
		{
			#region Constructors
			public ZipUpdate(string fileName, ZipEntryM entry)
			{
			...
			{
				command_ = UpdateCommand.Add;
				entry_ = new ZipEntryM(entryName);
			...
			public ZipUpdate(IStaticDataSource dataSource, ZipEntryM entry)
			{
				command_ = UpdateCommand.Add;
				entry_ = entry;
				dataSource_ = dataSource;
			...
			public ZipUpdate(ZipEntryM original, ZipEntryM updated)
			{
				//throw new ZipException("Modify not currently supported");
				/*
					command_ = UpdateCommand.Modify;
					entry_ = ( ZipEntryM )original.Clone();
					outEntry_ = ( ZipEntryM )updated.Clone();
				*/
			}

			public ZipUpdate(UpdateCommand command, ZipEntryM entry)
			{
				command_ = command;
				entry_ = (ZipEntryM)entry.Clone();
			}
			...
			public ZipUpdate(ZipEntryM entry)
			...
			/// <summary>
			/// Get the <see cref="ZipEntryM"/> for this update.
			/// </summary>
			...
			/// <summary>
			/// Get the <see cref="ZipEntryM"/> that will be written to the updated/new file.
			/// </summary>
			public ZipEntryM OutEntry {
				get {
					if (outEntry_ == null) {
						outEntry_ = (ZipEntryM)entry_.Clone();
					}
		...
		void ReadEntries()
		{
			...
			entries_ = new ZipEntryM[entriesForThisDisk];
			...
			//	int internalAttributes = ReadLEUshort();  // Not currently used
				...
				var entry = new ZipEntryM(name, versionToExtract, versionMadeBy, (CompressionMethod)method);
		...
		/// </exception>
		long LocateEntry(ZipEntryM entry)
		{
			return TestLocalHeader(entry, HeaderTest.Extract);
		}

		Stream CreateAndInitDecryptionStream(Stream baseStream, ZipEntryM entry)
		{
...
//					if (saltIn != saltLen)
		...
		Stream CreateAndInitEncryptionStream(Stream baseStream, ZipEntryM entry)
		{
			CryptoStream result = null;
			if ((entry.Version < ZipConstants.VersionStrongEncryption)
				|| (entry.Flags & (int)GeneralBitFlags.StrongEncryption) == 0) {
		...
		static void CheckClassicPassword(CryptoStream classicCryptoStream, ZipEntryM entry)
		{
		...
		// Hint: Set always ZipEntryM size before they are added to an archive and this setting isnt needed.
		...
		bool commentEdited_;
		IEntryFactory updateEntryFactory_ = new ZipEntryMFactory();
		#endregion
		#endregion
		...
		/// <summary>
		/// An <see cref="IEnumerator">enumerator</see> for <see cref="ZipEntryM">Zip entries</see>
		/// </summary>
		class ZipEntryMEnumerator : IEnumerator
		{
			#region Constructors
			public ZipEntryMEnumerator(ZipEntryM[] entries)
			{
				array = entries;
			}
	...
	public interface IDynamicDataSource
	{
		...
		/// Get a data source.
		/// </summary>
		/// <param name="entry">The <see cref="ZipEntryM"/> to get a source for.</param>
		/// <param name="name">The name for data if known.</param>
		/// <returns>Returns a <see cref="Stream"/> to use for compression input.</returns>
		/// <remarks>Ideally a new stream is created and opened to achieve this, to avoid locking problems.</remarks>
		Stream GetSource(ZipEntryM entry, string name);
	}
	...
	public class DynamicDiskDataSource : IDynamicDataSource
	{
		...
		public Stream GetSource(ZipEntryM entry, string name)
		{
			Stream result = null;
