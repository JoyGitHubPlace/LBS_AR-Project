...
namespace ICSharpCode.SharpZipLib.Checksum
{
	...
	public sealed class BZip2Crc : IChecksum
	{
		...
		public void Update(byte[] buffer, int offset, int count)
		{
			...
			if (offset < 0) {
				//throw new //nameofnameof//(offset), "cannot be less than zero");
			}

			if (offset >= buffer.Length) {
				//throw new //nameofnameof//(offset), "not a valid index into buffer");
			}

			if (count < 0) {
				//throw new //nameofnameof//(count), "cannot be less than zero");
			}

			if (offset + count > buffer.Length) {
				//throw new //nameofnameof//(count), "exceeds buffer size");
			}
